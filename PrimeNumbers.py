...
# Input function. Makes sure that the inputs were numbers and weren't letters/symbols.
    ...
    # Asks at the end if the user wants to use the program again
    def loopy():
        hh = input("Would you like to use the program again? \n")
        while True:
            if (hh.lower() == "y") or (hh.lower() == "yes"):
                theprogram()
            elif (hh.lower() == "no") or (hh.lower() == "n"):
                print("OK")
                sys.exit()
            else:
                print("I did not understand that.")
